...
export const MPHomePage: React.FC = () => {
  ...
  const banners = [
    { id: 1, title: '中秋关怀 · 企采礼包专场', desc: '全额福利卡扣减 · 免费开票直达', color: 'from-[#143A8F] to-[#1F5EFF]' },
    { id: 2, title: '品牌代金券 · 凭码即刻核销', desc: '星巴克/猫眼电影/肯德基 协议价8折起', color: 'from-[#FF7A00] to-amber-600' },
    { id: 3, title: '米面粮油与生鲜劳保', desc: '有机五常大米 · 产地直供免运费', color: 'from-emerald-700 to-teal-600' }
  ];
  ...
  const feedProducts = MOCK_PRODUCTS.slice(0, 8);
    ...
    <div className="bg-[#F5F7FA] min-h-full flex flex-col font-sans text-gray-800">
      ...
      <div className="bg-[#143A8F] px-3 pb-3 pt-1">
        <div className="relative flex items-center">
          ...
          <button
            ...
            className="absolute right-1 top-1/2 -translate-y-1/2 bg-[#1F5EFF] hover:bg-blue-700 text-white font-bold text-xs px-3 py-1 rounded-full cursor-pointer"
          >
            搜索
          </button>
        </div>
      ...
      <div className="px-3 -mt-1.5 z-10">
        <div className="bg-white rounded-2xl p-3 shadow-md border border-blue-100 flex items-center justify-between divide-x divide-gray-100">
          <div
            onClick={() => setMpPage('profile')}
            className="flex-1 pr-2 flex items-center gap-2 cursor-pointer active:opacity-70 transition-opacity"
          >
            ...
            <div>
              ...
              <div className="text-sm font-black text-[#1F5EFF] font-mono">
          ...
          <div
            onClick={() => setMpPage('profile')}
            className="flex-1 pl-3 flex items-center gap-2 cursor-pointer active:opacity-70 transition-opacity"
          >
      ...
      <div className="px-3 mt-3">
        <div className={`relative rounded-2xl overflow-hidden shadow-sm bg-gradient-to-r ${banners[activeBanner].color} p-4 text-white min-h-[110px] flex flex-col justify-between`}>
          <div>
            <span className="inline-block bg-white/20 text-yellow-300 text-[9px] font-bold px-2 py-0.5 rounded-full mb-1 border border-white/20">
              微信小程序企业专享
            </span>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-white/10">
            <button
              onClick={() => setMpPage('category')}
              className="bg-white text-[#143A8F] font-bold text-[10px] px-3 py-1 rounded-full flex items-center gap-0.5 shadow-xs cursor-pointer"
            >
              <span>立即去兑换</span>
              ...
              {banners.map((_, i) => (
                <button
                  ...
                  className={`h-1 rounded-full transition-all cursor-pointer ${
                    activeBanner === i ? 'w-4 bg-yellow-300' : 'w-1 bg-white/40'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl p-3 shadow-xs border border-gray-100">
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-gray-100">
            <div className="flex items-center gap-1.5">
              <span className="bg-[#143A8F] text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                企采协议
              </span>
            ...
            <button
              onClick={() => setMpPage('category')}
              className="text-[10px] text-[#1F5EFF] font-bold flex items-center"
            >
              <span>查看全部</span>
              <ChevronRight className="w-3 h-3" />
            </button>
          ...
          <div className="grid grid-cols-2 gap-2">
            {enterpriseExclusives.map(p => (
              <div
                ...
                className="bg-gray-50/80 rounded-xl p-2 flex gap-2 border border-gray-100 cursor-pointer active:bg-blue-50/50 transition-colors"
              >
                ...
                <div className="overflow-hidden flex flex-col justify-between flex-1">
                  ...
                  <div>
                    <span className="text-[9px] text-[#1F5EFF] bg-blue-50 font-bold px-1 py-0.2 rounded">
                      省¥{p.enterpriseSubsidyAmount}
                    </span>
                    <div className="text-xs font-black text-[#E5484D] font-mono mt-0.5">
                      ¥{p.price}
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50/60 rounded-2xl p-3 border border-blue-100">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] bg-blue-100 text-[#1F5EFF] font-bold px-1.5 py-0.2 rounded-full">
                免运费 · 到店出示二维码
              </span>
            </div>
            <button
              onClick={() => triggerPendingFeature('微信小程序 LBS 位置定位', '定位附近的加盟美发、烘焙甜品、健身房核销门店。')}
              className="text-[10px] text-gray-500 hover:text-blue-600 flex items-center"
            >
              定位: 北京朝阳 &gt;
            </button>
          </div>
          ...
          <div className="space-y-2">
            {nearbyServices.map(p => (
              <div
                key={p.id}
                onClick={() => setMpPage('detail', p.id)}
                className="bg-white rounded-xl p-2.5 flex items-center justify-between gap-2 shadow-xs border border-gray-100 cursor-pointer"
              >
                <div className="flex items-center gap-2.5 overflow-hidden">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <div className="text-[10px] text-gray-500 truncate mt-0.5">
                      {p.applicableStoreName || '包含朝阳区国贸店、三里屯店等28家门店'}
                    </div>
