        ...
        <div className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap text-[10px] text-gray-600 no-scrollbar">
          ...
          {currentCategory.hotKeywords.map(kw => (
            <button
              ...
              className="bg-blue-50 text-[#1F5EFF] border border-blue-100 px-2 py-0.5 rounded-full font-medium cursor-pointer"
            >
              {kw}
            </button>
          ))}
          ...
          {MOCK_CATEGORIES.map(cat => {
            ...
            return (
              <button
                ...
                className={`w-full py-3.5 px-2 text-left relative transition-colors flex flex-col gap-0.5 cursor-pointer ${
                  isActive
                    ? 'bg-white font-bold text-[#1F5EFF] shadow-xs'
                    : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'
                }`}
              >
                {isActive && (
                  <span className="absolute left-0 top-0 bottom-0 w-1 bg-[#1F5EFF] rounded-r" />
                )}
                <span className="truncate">{cat.name}</span>
                <span className="text-[9px] text-gray-400 truncate font-normal">
                  {cat.hotKeywords[0]}
                </span>
              </button>
            );
        ...
        <div className="flex-1 bg-white p-2.5 overflow-y-auto space-y-3">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-2.5 rounded-xl border border-blue-100/80 flex items-center justify-between">
            <div>
              ...
              <div className="text-[10px] text-gray-500 mt-0.5">
                企采协议补贴 · 共 {filteredProducts.length} 件福利卡可兑商品
              </div>
            </div>
            <span className="text-[9px] bg-[#1F5EFF] text-white font-bold px-2 py-0.5 rounded-full">
              全额包邮
            </span>
          </div>
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-12 text-gray-400 space-y-2">
              ...
              <button
                onClick={() => setKeyword('')}
                className="text-xs text-[#1F5EFF] underline font-bold"
              >
                清除关键字重试
              </button>
            </div>
            ...
            <div className="space-y-2.5 pb-8">
              {filteredProducts.map(p => (
                <div
                  ...
                  className="bg-white rounded-xl p-2 flex gap-2.5 border border-gray-100 shadow-2xs hover:border-blue-200 transition-all cursor-pointer active:bg-gray-50"
                >
                  <img
                    src={p.imageUrl}
                    alt={p.title}
                  ...
                  <div className="flex-1 overflow-hidden flex flex-col justify-between">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="text-[9px] bg-red-50 text-[#E5484D] font-bold px-1 py-0.2 rounded">
                          协议价省¥{p.enterpriseSubsidyAmount}
                        </span>
                        <span className="text-[9px] bg-blue-50 text-[#1F5EFF] font-bold px-1 py-0.2 rounded">
                          支持福利卡
                        </span>
                      </div>
