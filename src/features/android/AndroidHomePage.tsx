      ...
      <div className="p-3 space-y-3 flex-1 overflow-y-auto pb-16">
        ...
        <div className="bg-gradient-to-br from-white to-blue-50/60 rounded-3xl p-3.5 shadow-sm border border-blue-100/80 space-y-3">
          <div className="flex items-center justify-between border-b border-gray-100 pb-2">
            ...
            <span className="text-[10px] bg-[#EAF1FF] text-[#1F5EFF] font-bold px-2 py-0.5 rounded-full">Material 3 B2B2C</span>
          </div>

          <div className="grid grid-cols-2 gap-2.5">
            <div onClick={() => setAndroidPage('profile')} className="bg-gradient-to-r from-[#1F5EFF] to-blue-700 text-white rounded-2xl p-3 shadow-xs cursor-pointer active:scale-98 transition-transform">
          ...
          {[
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full whitespace-nowrap transition-all cursor-pointer ${
                activeChip === chip.id ? 'bg-[#1F5EFF] text-white shadow-md shadow-blue-500/20' : 'bg-white text-gray-700 border border-gray-200/80 hover:bg-gray-50'
              }`}
            >
              {chip.label}
            </button>
        ...
        <div className="bg-gradient-to-r from-gray-900 via-indigo-950 to-blue-950 text-white rounded-3xl p-4 shadow-md space-y-2 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-white/10 relative z-10">
            <button onClick={() => setAndroidPage('search')} className="bg-white text-[#143A8F] font-bold text-xs px-4 py-1.5 rounded-full flex items-center gap-1 shadow-sm cursor-pointer">
              <span>进入协议专区</span>
              ...
              {feedProducts.map((p) => (
                <div key={p.id} onClick={() => setAndroidPage('detail', p.id)} className="bg-white rounded-3xl p-2.5 shadow-2xs border border-gray-100 flex flex-col justify-between cursor-pointer active:scale-98 transition-transform">
                  <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 mb-2">
                    ...
                    <span className="absolute top-2 left-2 bg-[#1F5EFF] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md shadow-xs">省¥{p.enterpriseSubsidyAmount}</span>
                  </div>
