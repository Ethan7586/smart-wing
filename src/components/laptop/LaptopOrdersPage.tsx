      ...
      <div className="max-w-[1240px] mx-auto pt-3 px-3 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
          ...
          <div className="md:col-span-3 bg-white border border-gray-200 rounded-lg p-2.5 shadow-2xs space-y-1 text-xs">
            <div className="font-bold text-gray-800 pb-2 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-400">
              订单状态筛选
            </div>
            ...
            {[
              ...
              <button
                ...
                className={`w-full text-left px-2.5 py-2 rounded font-bold cursor-pointer transition-colors flex items-center justify-between ${
                  activeStatus === tab.id
          ...
          <div className="md:col-span-9 space-y-2.5">
            {filteredOrders.map(order => (
              <div
                key={order.orderId}
                className="bg-white border border-gray-200 rounded-lg shadow-2xs overflow-hidden text-xs"
              >
                {/* 订单头部信息 */}
                <div className="bg-gray-50 px-3 py-2 border-b border-gray-200 flex items-center justify-between text-[11px] text-gray-600 flex-wrap gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="bg-blue-50 text-[#1F5EFF] font-bold px-1.5 py-0.2 rounded">
                      {order.supplierName || '平台自营仓'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[#E5484D]">
                      {order.status === 'completed'
                        ? '已完成'
                        : order.status === 'pending_shipment' || order.status === 'pending_receipt'
                        ? '运输中'
                        : '待付款'}
                    </span>
                  </div>
                    ...
                    <div key={item.product.id} className="py-2 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="font-bold text-gray-800 truncate text-xs">
                            {item.product.title}
                          </div>
                          <div className="text-[10px] text-gray-400 mt-0.5">
                            规格：标准企业版 × {item.quantity}
                          </div>
                        </div>
                      </div>
                ...
                <div className="bg-gray-50/60 p-2.5 border-t border-gray-100 flex items-center justify-between flex-wrap gap-2 text-[11px]">
                  <div className="flex items-center gap-2 text-gray-600">
                    <span>总额: <strong className="text-gray-900">¥{order.totalAmount.toFixed(2)}</strong></span>
                    <span>·</span>
                    <span className="text-[#143A8F] font-bold">
                      福利卡扣减: -¥{order.welfareDeduction.toFixed(2)}
                    </span>
                  </div>
