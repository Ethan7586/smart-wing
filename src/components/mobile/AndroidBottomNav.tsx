          ...
          >
            ...
            <div
              className={`px-4 py-1 rounded-full transition-all duration-200 flex items-center justify-center relative ${
                isActive ? 'bg-[#EAF1FF] text-[#1F5EFF]' : 'text-gray-500 group-hover:text-gray-800'
              }`}
            >
              <Icon className={`w-5 h-5 transition-transform ${isActive ? 'scale-105 stroke-[2.5]' : 'stroke-2'}`} />

              {tab.id === 'checkout' && cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#E5484D] text-white font-bold text-[9px] min-w-[15px] h-[15px] rounded-full flex items-center justify-center px-1 shadow-xs">
                  {cartCount}
                </span>
              )}
            </div>
            ...
            <span
              className={`text-[10px] mt-0.5 tracking-tight ${
                isActive ? 'font-bold text-[#1F5EFF]' : 'font-medium text-gray-600'
              }`}
            >
              {tab.label}
            </span>
