...
const items: Array<{ page: PageRoute; label: string; icon: React.ComponentType<{ className?: string }> }> = [
...
export const MobileBottomNav: React.FC = () => {
  ...
  return (
    <nav
      aria-label="移动端主导航"
      className="md:hidden fixed inset-x-0 bottom-0 z-50 border-t border-gray-200 bg-white/95 backdrop-blur shadow-[0_-4px_18px_rgba(15,23,42,0.08)] pb-[env(safe-area-inset-bottom)]"
    >
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center gap-1 text-[11px] ${
                active ? 'text-[#1F5EFF] font-bold' : 'text-gray-500'
              }`}
            >
              <Icon className="w-5 h-5" />
              <span>{item.label}</span>
              {item.page === 'cart' && cartCount > 0 && (
                <span className="absolute top-1.5 left-1/2 ml-1 rounded-full bg-[#FF7A00] px-1.5 text-[9px] leading-4 text-white">
