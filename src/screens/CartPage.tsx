  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-[1280px] mx-auto px-4 py-12 text-center font-sans space-y-4">
        <div className="w-20 h-20 bg-blue-50 text-[#1F5EFF] rounded-full flex items-center justify-center mx-auto shadow-xs">
        ...
        <button onClick={() => navigateTo('home')} className="bg-[#1F5EFF] hover:bg-blue-700 text-white font-bold text-xs px-6 py-2.5 rounded shadow-sm transition-colors">
          返回商城首页选购
        </button>
      </div>
    );
      ...
      <div className="flex items-center justify-between bg-white border border-gray-200 rounded-md p-4 shadow-xs text-xs">
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer font-bold text-gray-800">
            <input type="checkbox" checked={isAllSelected} onChange={(e) => toggleSelectAllCart(e.target.checked)} className="w-4 h-4 text-[#1F5EFF] rounded" />
            <span>全选所有商品 ({cart.length}件)</span>
          </label>
        </div>

        <div className="flex items-center gap-2 text-gray-500">
          <span className="text-[#1F5EFF] font-bold">1. 选购与勾选</span>
            ...
            <div key={group.supplierName} className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-xs">
              ...
              <div className="bg-gray-50 border-b border-gray-200 px-4 py-3 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="bg-[#1F5EFF] text-white text-[10px] font-bold px-1.5 py-0.5 rounded">{group.supplierName}</span>
              ...
              <div className="divide-y divide-gray-100">
                {group.items.map((item) => (
                  <div key={item.id} className={`p-4 flex items-center gap-4 text-xs transition-colors ${item.selected ? 'bg-blue-50/20' : 'bg-white'}`}>
                    <input type="checkbox" checked={item.selected} onChange={() => toggleCartItemSelected(item.id)} className="w-4 h-4 text-[#1F5EFF] rounded cursor-pointer" />
                    ...
                    <div className="flex-1 space-y-1">
                      <h3 onClick={() => navigateTo('detail', { productId: item.product.id })} className="font-bold text-gray-900 hover:text-[#1F5EFF] cursor-pointer line-clamp-1">
                        {item.product.title}
                      </h3>
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-gray-400 pt-1">
                        ...
                        {item.product.allowedAccounts.includes('welfare') && <span className="text-[#1F5EFF] font-semibold">支持福利卡抵扣</span>}
      ...
      <div className="sticky bottom-0 bg-white border border-gray-300 rounded-md shadow-2xl p-4 z-30 flex flex-col md:flex-row items-center justify-between gap-4 text-xs">
        <div className="flex items-center gap-4">
          ...
          <span className="text-gray-500">
            已选中 <strong className="text-[#1F5EFF]">{selectedItems.length}</strong> 件商品
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-6">
          <div className="text-right space-y-0.5">
            ...
            <div className="flex items-center justify-end gap-3 text-[11px] text-gray-500">
              <span>
                福利卡预扣: <strong className="text-[#1F5EFF]">¥{estWelfareDeduct.toFixed(2)}</strong>
              </span>
              <span>
          ...
          <button
            ...
            className="bg-[#1F5EFF] hover:bg-blue-700 disabled:opacity-40 text-white font-black px-8 py-3 rounded text-sm shadow-md transition-colors cursor-pointer flex items-center gap-2"
          >
