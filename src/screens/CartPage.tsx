    ...
    return (
      <div className="max-w-[1280px] mx-auto px-4 py-12 text-center font-sans space-y-4">
        ...
        <button
          ...
          className="bg-[#1F5EFF] hover:bg-blue-700 text-white font-bold text-xs px-6 py-2.5 rounded shadow-sm transition-colors"
        >
          返回商城首页选购
        </button>
      </div>
        ...
        {groupedCart.map(group => {
          ...
          return (
            <div
              ...
              className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-xs"
            >
              {/* 供应商分组包头 */}
              ...
              <div className="divide-y divide-gray-100">
                {group.items.map(item => (
                  <div
                    key={item.id}
                    className={`p-4 flex items-center gap-4 text-xs transition-colors ${
                      item.selected ? 'bg-blue-50/20' : 'bg-white'
                    }`}
                  >
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 text-[#1F5EFF] rounded cursor-pointer"
                    />
                    ...
                    <div className="flex-1 space-y-1">
                      <h3
                        onClick={() => navigateTo('detail', { productId: item.product.id })}
                        className="font-bold text-gray-900 hover:text-[#1F5EFF] cursor-pointer line-clamp-1"
                      >
                        {item.product.title}
                      </h3>
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-gray-400 pt-1">
                        ...
                        {item.product.allowedAccounts.includes('welfare') && (
                          <span className="text-[#1F5EFF] font-semibold">支持福利卡抵扣</span>
                        )}
                        {item.product.allowedAccounts.includes('meal') && (
                          <span className="text-[#FF7A00] font-semibold">支持餐卡</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="text-right space-y-1 min-w-[120px]">
                      <div className="text-[#FF7A00] font-black text-sm">
                    ...
                    <div className="text-right min-w-[100px] font-black text-[#FF7A00] text-sm">
            ...
            <div className="flex items-center justify-end gap-3 text-[11px] text-gray-500">
              ...
              {estWechatTopUp > 0 && (
                <span className="text-red-600 font-bold">
                  微信需补差: ¥{estWechatTopUp.toFixed(2)}
                </span>
              )}
            </div>
          </div>
