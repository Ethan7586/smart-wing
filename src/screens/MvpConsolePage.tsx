...
export const MvpConsolePage: React.FC = () => {
  ...
  const metrics = [
    ...
    { label: "福利余额", value: `¥${user.welfareBalance.toFixed(2)}`, icon: ShieldCheck, color: "text-emerald-600" },
    { label: "餐卡余额", value: `¥${user.mealBalance.toFixed(2)}`, icon: Users, color: "text-purple-600" },
  ];
    ...
    <div className="max-w-[1280px] mx-auto px-4 py-6 space-y-5">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            用于甲方阶段验收、数据链路检查和第三方接入准备，不等同于正式运营后台。
          </p>
        </div>
        <button
          onClick={() => void refresh()}
          disabled={loading}
          className="bg-[#143A8F] disabled:bg-blue-300 text-white text-xs font-bold rounded px-4 py-2 flex items-center gap-2"
        >
      ...
      {sessionStatus !== "authenticated" && (
        <div className="bg-amber-50 border border-amber-200 rounded p-4 text-xs text-amber-900">
          当前为访客预览。请先通过页面顶部的“登录MVP”建立安全会话，才能核验账户和订单数据。
        </div>
      )}
      ...
      <div className="bg-[#EAF1FF] border border-blue-200 rounded-md p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {[
            ...
            <button
              key={page}
              onClick={() => navigateTo(page as "category" | "cart" | "orders" | "balance" | "user-center")}
