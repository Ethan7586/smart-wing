    ...
    <div className="max-w-[1280px] mx-auto px-4 py-4 space-y-4 font-sans text-xs">
      ...
      <div className="bg-white border border-gray-200 rounded-md p-4 shadow-xs flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 font-bold">
          {[
            { id: 'unused', label: `未使用 (${coupons.filter(c => c.status === 'unused').length})` },
            { id: 'used', label: `已核销 (${coupons.filter(c => c.status === 'used').length})` },
            { id: 'expired', label: `已失效 (${coupons.filter(c => c.status === 'expired').length})` }
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCoupons.map(coupon => {
            ...
            return (
              <div
                key={coupon.id}
                className={`bg-white border rounded-md shadow-xs overflow-hidden transition-all flex flex-col justify-between ${
                  isUnused ? 'border-orange-300 hover:shadow-md' : 'border-gray-200 opacity-60'
                }`}
              >
                {/* 卡头标识 */}
                <div className={`p-4 text-white flex items-center justify-between ${
                  ...
                  <div className="flex items-center gap-2">
                    <span className="bg-white/20 px-2 py-0.5 rounded text-[10px] font-bold">
                      {coupon.type === 'movie_ticket' ? '影城门票' : coupon.type === 'supermarket' ? '电子商超卡' : '门店核销券'}
                    </span>
                ...
                <div className="p-4 space-y-3 flex-1">
                  <div className="text-gray-600 leading-relaxed font-medium">
                    {coupon.usageRules?.join(' · ') || '企业福利专属兑换券'}
                  </div>
                  ...
                  <div className="text-[11px] text-gray-400 space-y-1 pt-2 border-t border-gray-100">
                    ...
                    <div className="font-mono text-gray-700 font-bold">
                      核销券码：{coupon.code}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 border-t border-gray-100 p-3 flex items-center justify-between">
                  ...
                  {isUnused ? (
                    <button
                      ...
                      className="bg-[#1F5EFF] hover:bg-blue-700 text-white font-bold px-4 py-1.5 rounded flex items-center gap-1 cursor-pointer shadow-2xs"
                    >
                      <QrCode className="w-3.5 h-3.5" /> 出示核销码 / 到店核销
                    </button>
                  ) : (
                    <span className="bg-gray-200 text-gray-600 px-3 py-1 rounded font-bold">
                      {coupon.status === 'used' ? '已于门店核销' : '已过期失效'}
                    </span>
                  )}
                </div>
              </div>
      ...
      {selectedCoupon && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in">
          <div className="bg-white rounded-md p-6 max-w-sm w-full space-y-4 text-center relative shadow-2xl">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded p-2 text-amber-900 font-mono font-black text-base tracking-widest">
              {selectedCoupon.code}
            </div>

            <div className="text-[11px] text-gray-400">
              请向门店收银员出示此码，或在合作小程序/猫眼APP内直接输入使用。
            </div>

            <div className="pt-2">
              <button
                onClick={() => handleSimulateVerification(selectedCoupon.id)}
                className="w-full bg-[#1F5EFF] hover:bg-blue-700 text-white font-black py-2.5 rounded text-xs transition-colors cursor-pointer shadow-md"
              >
                模拟门店扫码核销 (完成核销)
              </button>
            </div>
