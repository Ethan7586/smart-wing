    ...
    <div className="max-w-[1024px] mx-auto px-4 py-4 space-y-4 font-sans text-xs">
      ...
      <div className="flex items-center justify-between text-xs">
        <button
          ...
          className="flex items-center gap-1 text-gray-600 hover:text-[#1F5EFF] font-bold cursor-pointer"
        >
      ...
      <div className="bg-white border border-gray-200 rounded-md p-6 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div>
            ...
            <div className="text-xs text-gray-400 mt-0.5">
              由【{order.supplierName}】承担采购与物流发货履约
            </div>
          </div>
          <span className="bg-blue-50 border border-blue-200 text-[#1F5EFF] font-bold text-xs px-3 py-1 rounded">
            集团福利协议保障订单
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 text-center text-xs pt-2">
          <div className="space-y-1">
            <div className="w-8 h-8 rounded-full bg-[#1F5EFF] text-white flex items-center justify-center mx-auto font-bold">
              1
            </div>
          ...
          <div className="space-y-1">
            ...
            <div className="text-[10px] text-gray-400">
              {order.expressCompany ? `${order.expressCompany}` : '备货排单中'}
            </div>
          </div>

          <div className="space-y-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto font-bold ${
              order.status === 'completed' ? 'bg-[#1F5EFF] text-white' : 'bg-gray-200 text-gray-600'
            }`}>
              4
            </div>
            <div className="font-bold text-gray-800">确认签收/归档</div>
            <div className="text-[10px] text-gray-400">
              {order.status === 'completed' ? '订单已完成' : '待签收'}
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-2">
              ...
              <div className="border-l-2 border-blue-500 pl-3 space-y-2 pt-2 text-[11px]">
                <div className="text-blue-700 font-bold">
                  【北京市】快递员正在派送中，请保持电话畅通。
                </div>
                <div className="text-gray-400">
                  【北京市西城区转运中心】已收入仓，准备发往派送点。
                </div>
                <div className="text-gray-400">
                  【平台直发仓】包裹已拣货打包完成，由顺丰速运揽收。
                </div>
              </div>
            </div>
          ) : (
            <div className="text-gray-500 py-4 text-center">
              订单正在仓库配货中，预计 24 小时内上传物流单号。
            </div>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-md p-5 shadow-xs space-y-4 text-xs">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded p-4 space-y-1.5 text-right font-medium text-gray-700">
          ...
          {order.payment.wechatPaid > 0 && (
            <div className="text-red-600">微信补差已付：¥{order.payment.wechatPaid.toFixed(2)}</div>
          )}
          <div className="text-sm font-black text-gray-900 pt-2 border-t border-gray-200">
            最终合并扣除金额：<span className="text-[#FF7A00]">¥{order.payment.finalPaidAmount.toFixed(2)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            ...
            className="border border-orange-300 text-orange-700 hover:bg-orange-50 font-bold px-4 py-2 rounded cursor-pointer"
          >
            申请售后/退款
          </button>
          <button
            onClick={() => showToast('电子发票PDF预检成功！正式版将在订单签收后同步至个人中心。', 'success')}
            className="bg-[#1F5EFF] text-white font-bold px-4 py-2 rounded cursor-pointer"
          >
            预览增值税发票
          </button>
        </div>
