  ...
  it('accepts a roster account and rejects former universal passwords', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ authorization: { membershipId: 'membership-test-buyer-001', target: 'storefront' }, entrances: { storefront: true, admin: false } }), { status: 200, headers: { 'content-type': 'application/json' } })
      )
    ...
    await expect(accepted).resolves.toMatchObject({ identifier: 'buyer001', entrances: { storefront: true, admin: false }, memberships: [{ id: 'membership-test-buyer-001', target: 'storefront' }] });
