      ...
      {paginatedProducts.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-md p-12 text-center space-y-3">
          ...
          <div className="flex items-center justify-center gap-2 pt-1">
            <button onClick={() => navigateTo('home')} className="bg-[#1F5EFF] text-white font-bold text-xs px-4 py-2 rounded">
              回到首页继续选购
            </button>
              ...
              <div key={p.id} className="bg-white border border-gray-200 rounded-md p-4 flex flex-col sm:flex-row items-center gap-4 hover:border-blue-400 shadow-xs transition-all">
                ...
                <div className="flex-1 space-y-1.5 text-left">
                  <div className="flex items-center gap-2">
                    <span className="bg-[#1F5EFF] text-white text-[10px] font-bold px-1.5 py-0.5 rounded">{p.supplierName}</span>
                    <span className="text-xs text-gray-400 font-medium">品牌: {p.brand}</span>
                  </div>

                  <h3 onClick={() => navigateTo('detail', { productId: p.id })} className="text-sm font-bold text-gray-900 cursor-pointer hover:text-[#1F5EFF]">
                    {p.title}
                  </h3>
        ...
        <div className="bg-white border border-gray-200 rounded-md p-4 flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {pageNumbers.map((page) => (
              <button key={page} onClick={() => setCurrentPageNum(page)} className={`w-7 h-7 rounded font-bold cursor-pointer ${currentPageNum === page ? 'bg-[#1F5EFF] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {page}
              </button>
            ))}
