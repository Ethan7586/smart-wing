    ...
    <div className="space-y-3 border-b border-slate-100 px-5 py-4">
      <div className="grid gap-3 xl:grid-cols-[minmax(240px,1.5fr)_minmax(140px,.7fr)_minmax(250px,1fr)_auto]">
        <label className="relative">
          ...
          <input
            ...
            className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs outline-none placeholder:text-slate-400 focus:border-[#1769ff] focus:ring-2 focus:ring-blue-100"
          />
        </label>
        <label>
          <span className="sr-only">订单状态</span>
          <select value={filters.status} onChange={(event) => set('status', event.target.value)} className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs text-slate-600 outline-none focus:border-[#1769ff]">
        ...
        <div className="flex gap-2">
          <button type="button" disabled={loading} onClick={onSearch} className="h-9 rounded-lg bg-[#1769ff] px-4 text-xs font-semibold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50">
            查询
          </button>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" disabled={loading} onClick={onExport} className="h-8 rounded-lg border border-[#bdd5ff] bg-blue-50 px-3 text-xs font-medium text-[#1769ff] hover:bg-blue-100 disabled:opacity-50">
            导出 CSV
          </button>
        </div>
      </div>
