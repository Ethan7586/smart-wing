...
export class WorkstationLoadBoundary extends React.Component<WorkstationLoadBoundaryProps, WorkstationLoadBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <section className="m-6 max-w-xl rounded-2xl border border-[var(--sw-danger)]/20 bg-[var(--sw-surface)] p-6 shadow-sm" role="alert">
          ...
          <button type="button" onClick={this.props.onReturnToCockpit} className="mt-5 rounded-xl bg-[var(--sw-brand)] px-4 py-2 text-xs font-semibold text-white hover:bg-[var(--sw-brand-hover)]">
            返回经营驾驶舱
          </button>
        </section>
      );
