...
const OUT = join(ROOT, 'apps/wechat-miniapp/miniprogram/styles');
...
${Object.entries(t.icon.sizes).map(([name, pt]) => `  --sw-icon-${name}: ${rpx(pt)};`).join('\n')}
  ...
  --sw-shadow-card: 0 2rpx 8rpx rgba(7, 24, 47, 0.04);
  --sw-shadow-overlay: 0 -4rpx 24rpx rgba(7, 24, 47, 0.08);
  --sw-shadow-membercode: 0 8rpx 24rpx rgba(31, 94, 255, 0.24);
...
writeFileSync(join(OUT, 'tokens.wxss'), tokensWxss);
writeFileSync(join(OUT, 'icons.wxss'), iconsWxss);
