export const RULES = ['hardcoded-color', 'font-size-floor', 'font-weight-ceiling', 'radius-off-scale', 'palette-drift', 'semantic-duplicate'];
...
const COLOR_UTILITIES = new Set(['bg', 'text', 'border', 'ring', 'outline', 'decoration', 'divide', 'shadow', 'accent', 'caret', 'fill', 'stroke', 'from', 'via', 'to']);
...
function radiusAllowed(value, allowed, origin) {
  ...
  if (/^var\(--sw-radius(?:-[^)]+)?\)$/.test(clean) || ['inherit', 'initial', 'revert', 'revert-layer', 'unset', '50%', '9999px'].includes(clean)) {
    return true;
  }
  const pieces = clean
    .replaceAll('_', ' ')
...
function roundedValue(utility) {
  ...
  const rest = utility.slice(8);
  if (rest.startsWith('[')) return rest;
  const parts = rest.split('-');
  if (DIRECTIONS.has(parts[0])) {
    parts.shift();
...
function scanUtility(context, findings, allowedRadii) {
  for (const token of tokens(context)) {
    ...
    const colour = utility.match(/^(?:bg|text|border(?:-[trblxyse]{1,2})?|ring(?:-offset)?|outline|decoration|divide|shadow|accent|caret|fill|stroke|from|via|to)-([a-z]+)-(?:\d{2,3})(?:\/\d{1,3})?$/);
    ...
    const arbitrary = utility.match(/^([a-z]+(?:-[a-z]+)*)-\[([\s\S]+)\](?:\/.+)?$/);
    const arbitraryBase = arbitrary?.[1].split('-')[0];
    if (arbitrary && COLOR_UTILITIES.has(arbitraryBase) && arbitraryBase !== 'text') {
