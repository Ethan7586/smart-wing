...
const SOURCE_ROOT = 'https://amazon-berkeley-objects.s3.us-east-1.amazonaws.com';
...
function categoryFor(item) {
  const source = [...(item.node ?? []).map((node) => node.node_name ?? node.path ?? ''), ...(item.product_type ?? []).map((type) => type.value ?? '')].join(' ').toLowerCase();
...
function compactItem(item) {
  ...
  const description = localized(item.product_description) || localized(item.bullet_point) || 'Amazon Berkeley Objects 非商业测试商品。';
  return {
...
async function main() {
  ...
  await writeFile(output, `${products.map((product) => JSON.stringify(product)).join('\n')}\n`, 'utf8');
