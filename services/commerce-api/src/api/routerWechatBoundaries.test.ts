...
describe('WeChat route authentication boundaries', () => {
  it.each(['/api/v1/auth/wechat/session', '/api/v1/auth/wechat/bind', '/api/v1/payments/wechat/notify'])(
    'dispatches public endpoint %s before user authentication',
    async (pathname) => {
  ...
  it('keeps WeChat prepay behind user authentication', async () => {
    const response = await routeApi(
      new Request('https://hbbtzn.com/api/v1/orders/order-1/payments/wechat/prepay', { method: 'POST' }),
      env
    );
